...
Bulk-photo uploads are staged as temporary objects in Supabase Storage. PostgreSQL manifests contain metadata only—never base64 content or raw image bytes. Student photos use the deterministic `<admission_number>.jpg` convention (with `.jpeg`, `.png`, or `.webp` also accepted) and are matched case-insensitively, after trimming only surrounding whitespace, within the authorized school. Preview performs no permanent student-photo writes. A student batch can be confirmed only when every relevant entry is uniquely matched and valid.

Student-photo ZIPs are limited to 25 MB compressed, 5,000 relevant entries, 100 MB total expanded data, a 100:1 per-entry compression ratio, and 5 MB per image. JPEG, PNG, and WebP are accepted, with decoded image content required to match the extension. Paths, nested entries, symbolic links, encrypted entries, corrupt archives/images, duplicate filenames, and multiple files resolving to one admission number are rejected or reported. Common `__MACOSX`, `.DS_Store`, and AppleDouble metadata are ignored.

On confirmation, the backend revalidates school/user ownership, preview status, admission-number-to-student identity, current photo state, managed temporary paths, and image data. It uploads every replacement object first, then changes all student rows, audit events, and the manifest in one PostgreSQL transaction. Any upload failure removes new objects; any database failure rolls back the transaction and removes all newly uploaded objects. Existing photos and staged objects are deleted only after the database commit, so cleanup failure can leave a recoverable orphan but cannot leave a student pointing at a missing replacement. Preview state is user- and school-scoped and expires after 24 hours; on-access cleanup removes expired staged objects.
