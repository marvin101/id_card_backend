...
- [ ] Open Cards and verify the Front/Back preview toggle for a duplex template. Export front-only and duplex PDFs for individual, filtered, selected-only, and Print Basket scopes, including missing-photo warnings and both portrait and landscape templates. Verify alternating front/back pages, long-edge and short-edge alignment, partial-sheet student pairing, one-card pages, A4/Letter portrait/landscape sheets, margins, spacing, optional crop marks, exact card size, and both PDF-page and physical-sheet counts.
