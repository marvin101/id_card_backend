...
- [ ] `python -m app.core.postgres_patch_level` confirms Supabase meets the 2026-08-13 PostgreSQL security baseline; record the version and result.
- [ ] Any Alembic-autogenerated revision was manually reviewed, including named CHECK constraints whose expressions are not compared by autogeneration.
- [ ] The intended application target is `e8f5b21c0d93`; review the complete chain including durable auth sessions, personnel custom fields/photos, import/grid support, credential compatibility, RLS, and downgrade behavior. Production remains at `d7e4a10b9c82` until this release step is explicitly authorized.
...
## Workday session and administration gates

- [ ] Migration `e8f5b21c0d93` is reviewed and applied before session-enabled backend code receives traffic.
- [ ] A valid refresh token rotates, a stale token revokes its session, and access tokens are rejected by `/auth/refresh`.
- [ ] Inactive/deleted users and revoked/expired sessions cannot refresh; password reset and deactivation revoke existing sessions.
- [ ] A long student/personnel/Grid/Designer workflow survives proactive and 401-driven renewal without losing unsaved state.
- [ ] Concurrent API requests perform one refresh, transient outages preserve retryable state, and logout clears/revokes both credentials.
- [ ] Platform Administration creates/edits/activates/deactivates schools and accounts, protects self/last-admin access, and manages multi-school roles without Swagger or direct SQL.
- [ ] The approved `campusid@proton.me` support address is present, and approved `PRIVACY_NOTICE` and `TERMS_NOTICE` build values replace the remaining launch placeholders.
- [ ] Disposable staging verifies student/personnel/logo upload, replace and delete, bulk temporary promotion and cleanup.